...
def _smoke_check_installed_tool() -> bool:
    info(f"Verifying {TOOL_BIN_NAME} entry point...")
    try:
        ...
        fail(f"'{TOOL_BIN_NAME} --help' timed out.")
        return False
    except OSError as exc:
        fail(f"Could not run {TOOL_BIN_NAME}: {exc}")
        return False

    if result.returncode != 0:
        fail(f"'{TOOL_BIN_NAME} --help' failed after installation.")
...
def _install_tool(uv_path: Path) -> bool:
    existing = shutil.which(TOOL_BIN_NAME)
    if existing:
        ok(f"'{TOOL_BIN_NAME}' already installed: {existing}")
        info("Skipping reinstall (run 'uv tool upgrade clickup-mcp-server' to update).")
        return _smoke_check_installed_tool()

    info("Installing clickup-mcp-server via uv tool install...")
    try:
        result = subprocess.run(
            [str(uv_path), "tool", "install", "--force", GIT_PACKAGE_REF],
            capture_output=True,
            text=True,
            timeout=120,
            check=False,
        ...
        fail("Installation timed out after 120 seconds.")
        return False
    except OSError as exc:
    ...
    if result.returncode != 0:
        fail("Failed to install clickup-mcp-server.")
        stderr = result.stderr.strip()
        if stderr:
            print(f"    {stderr}")
        return False

    installed = shutil.which(TOOL_BIN_NAME)
    if not installed:
        fail(f"'{TOOL_BIN_NAME}' not found on PATH after install.")
        info("Ensure ~/.local/bin is on your PATH.")
        return False

    ok(f"Installed: {installed}")
    return _smoke_check_installed_tool()
...
def _op_run(args: list[str]) -> subprocess.CompletedProcess[str]:
    try:
        ...
        return subprocess.CompletedProcess(
            args=["op", *args], returncode=1, stdout="", stderr=str(exc)
        )
...
_WORKSPACE_HINT = (
    "Settings → Workspaces, or from any ClickUp URL: app.clickup.com/{workspace_id}/..."
)
_SPACE_HINT = "Click a Space → ID is in the URL: /s/{space_id}/..."
_FOLDER_HINT = (
    "The folder containing your sprint lists. Find it via "
    "get_workspace_hierarchy tool or URL."
)
_FIELD_HINT = (
    "Custom field ID for your Component/Team dropdown. "
...
def setup_claude_code(
    token: str, env_vars: dict[str, str], tool_path: str = TOOL_BIN_NAME
) -> bool:
    ...
    cmd.extend(["-s", "user", MCP_NAME, "--", tool_path])
...
def setup_claude_desktop(
    ...
    if config_path.is_file():
        try:
            ...
            info("Fix or remove the file manually, then re-run setup.")
            return False
...
def main() -> int:
    ...
    tool_path = shutil.which(TOOL_BIN_NAME) or TOOL_BIN_NAME
    ...
    if args.code or args.both:
        if not setup_claude_code(token, env_vars, tool_path):
            rc = 1
        print()

    desktop_configured = False
    if args.desktop or args.both:
        if not setup_claude_desktop(token, env_vars, tool_path):
            rc = 1
        else:
    ...
    if rc == 0:
        ...
        print()
        print(f"  To upgrade later: {bold('uv tool upgrade clickup-mcp-server')}")
    else:
