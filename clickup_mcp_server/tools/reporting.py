...
async def _fetch_task_pr_links(task_id: str) -> list[str] | None:
    """Return the task's PR links, or None if the comments could not be read.

    None and [] must stay distinguishable: the caller reports "no PR link found"
    as a finding, which would otherwise be asserted on a failed request.
    """
    try:
        ...
        return links
    except Exception:  # noqa: BLE001 — the report must survive an unreadable comment thread
...
def register_reporting_tools(server: MCPServer) -> None:
    @server.tool(
    ...
    async def get_sprint_report(
        ...
        pr_links_unreadable: set[str] = set()
        if include_pr_links:
            ...
            async def _bounded_fetch(tid: str) -> tuple[str, list[str] | None]:
                async with sem:
        ...
        for task in tasks:
            ...
            if (
                include_pr_links
                and "review" in task.status.lower()
                and not pr_links
                and task.id not in pr_links_unreadable
            ):
                _add_at_risk(
                    at_risk_map,
                    task.id,
