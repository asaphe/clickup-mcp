...
def register_task_tools(server: MCPServer) -> None:
    ...
    async def create_task(
        ...
        Args:
            ...
            status: Initial status. Omitted unless given, so the target list's own
                default status applies. ClickUp configures status sets per list, so
                no value is safe to impose on an arbitrary list_id.
            points: Story points.
    ...
    async def create_sprint_task(
        ...
        Args:
            ...
            points: Story points. Omitted unless given.
    ...
    async def get_my_tasks(
        ...
        Args:
            query: Text search query (client-side filtered — see search_tasks).
            status: Filter by status(es).
            list_id: Limit to a specific list.
            date_created_after: ISO date or Unix millis — only tasks created after this.
            date_updated_after: ISO date or Unix millis — only tasks updated after this.
            ...
            subtasks: Include subtasks in results (default: true). Inverts
                search_tasks' own default, since a personal task list is mostly
                subtasks; pass false to match search_tasks.
        """
        user = await get_current_user_cached()
    ...
    async def get_list_tasks(
        ...
        Args:
            ...
            include_closed: Include closed/done tasks (default: false).
            subtasks: Include subtasks in results (default: true). Inverts
                search_tasks' own default; pass false to match it.
        """
        validate_list_id(list_id)
        base_params: dict[str, str] = {}
        if subtasks:
            base_params["subtasks"] = "true"
        if include_closed:
    ...
    async def bulk_update_tasks(
        ...
        for tid in task_ids:
            ...
            except Exception as exc:  # noqa: BLE001 — per-item isolation: one bad task must not abort the batch
    ...
    async def ensure_task_fields(
        ...
        for tid in task_ids:
            ...
            except Exception as exc:  # noqa: BLE001 — per-item isolation: one bad task must not abort the check
                issues.append(
                    FieldCheckResult(
                        task_id=tid,
                        custom_id=None,
