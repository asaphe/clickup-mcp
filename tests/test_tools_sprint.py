...
class TestSprintDetection:
    ...
    async def test_current_sprint_by_date(self) -> None:
        ...
        with (
            patch.object(sprint_mod.clickup_client, "get", side_effect=mock_get),
            patch("clickup_mcp_server.tools.sprint.time") as mock_time,
        ):
            mock_time.time.return_value = 1709500000.0
            result = await sprint_mod.get_current_sprint_cached()

        assert result.list_id == "444444444"
        assert result.name == "Sprint 31"
        sprint_mod._sprint_task = None

    @pytest.mark.asyncio
    async def test_non_list_folders_raises(self) -> None:
        from clickup_mcp_server.tools import sprint as sprint_mod
        ...
        async def mock_get(
            ...
            return _mock_response({"folders": {"not": "a list"}})

        with (
            patch.object(sprint_mod.clickup_client, "get", side_effect=mock_get),
            pytest.raises(TypeError, match="Unexpected /space folder response format"),
        ):
            await sprint_mod.get_current_sprint_cached()
