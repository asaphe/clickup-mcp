...
class TestTransportRetryIdempotency:
    @pytest.mark.asyncio
    async def test_post_is_not_retried_on_transport_error(
        ...
        with (
            patch.object(type(client.client), "request", new=boom),
            patch.object(asyncio, "sleep", new=no_sleep),
            pytest.raises(httpx.ConnectError),
        ):
            await client.post("/list/1/task", json_data={"name": "x"})

        # A timed-out create may already have been applied server-side.
        assert attempts == 1

    @pytest.mark.asyncio
    async def test_get_is_still_retried_on_transport_error(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        import asyncio
        from unittest.mock import patch
